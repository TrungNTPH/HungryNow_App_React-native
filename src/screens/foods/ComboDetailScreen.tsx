...
import QuantitySelectionModal from '../../modals/QuantitySelectionModal';
...
const computeAvailableFromCombo = (combo: any): number => {
  if (!Array.isArray(combo?.foods) || combo.foods.length === 0) return 0;
  let minQuota = Infinity;
  for (const f of combo.foods) {
    const stock = Number(f?.foodId?.quantity ?? 0);
    const need = Number(f?.quantity ?? 0);
    if (need <= 0) continue;
    const canMake = Math.floor(stock / need);
    minQuota = Math.min(minQuota, canMake);
  }
  if (!Number.isFinite(minQuota)) return 0;
  return Math.max(0, minQuota);
};

const ComboDetailScreen = ({ navigation, route }: any) => {
  const { combo, suggestedCombos } = route.params;
  ...
  const avgRating = avgStarsMap?.[combo._id] || 0;
  const totalRatings = totalRatingsMap?.[combo._id] || 0;
  ...
  const [liveAvailable, setLiveAvailable] = useState<number>(
    computeAvailableFromCombo(combo),
  );
  const [busy, setBusy] = useState(false);

  useEffect(() => {
    setLiveAvailable(computeAvailableFromCombo(combo));
  }, [combo?._id]);

  const isDiscontinued = combo?.status === 'unavailable';

  const filteredSuggestedCombos = useMemo(() => {
    const base = (suggestedCombos || []).filter(
      (item: { _id: any }) => item._id !== combo._id,
    );
    ...
    return merged;
  }, [suggestedCombos, extraSuggests, combo._id]);
  ...
  const loadData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        await Promise.all([
          dispatch(
            fetchRatingsThunk({ itemId: combo._id, itemType: 'Combo' }),
          ).unwrap(),
          dispatch(
            fetchAverageStarsThunk({ itemId: combo._id, itemType: 'Combo' }),
          ).unwrap(),
        ]);
      } catch {
        showError('Failed to refresh combo details.');
      } finally {
        setRefreshing(false);
        setBusy(false);
      }
  ...
  useFocusEffect(
    useCallback(() => {
      if (combo?._id) loadData();
    }, [loadData]),
  );

  useEffect(() => {
    if (comboError && !refreshing) {
      showError('Failed to load combo details');
      dispatch(clearComboMessages());
    }
  }, [comboError, refreshing, dispatch]);

  useEffect(() => {
    if (comboSuccessMessage && !refreshing) {
      showSuccess(comboSuccessMessage);
      dispatch(clearComboMessages());
    }
  }, [comboSuccessMessage, refreshing, dispatch]);

  const fetchExtraSuggests = useCallback(async () => {
    try {
      setSuggestLoading(true);
      const categoryId =
        typeof combo?.categoryId === 'string'
          ? combo.categoryId
          : combo?.categoryId?._id;
      const params: any = { type: 'Combo', page: 1, limit: 12 };
      ...
      const cleaned = items
        ...
        .filter((x: any) => x?._id && x._id !== combo._id);
  ...
  useEffect(() => {
    if ((suggestedCombos?.length || 0) < 4) {
      fetchExtraSuggests();
    } else {
      setExtraSuggests([]);
    }
  }, [suggestedCombos, fetchExtraSuggests]);
  ...
  const handleAddToCartPress = useCallback(
    async (action: () => void) => {
      ...
      if (isDiscontinued) {
        showError('This combo is discontinued.');
        return;
      }
      if (liveAvailable <= 0) {
        showError('This combo is out of stock.');
        return;
      }
      action();
    },
    [auth.accesstoken, liveAvailable, isDiscontinued],
  );

  const handleAddToCart = useCallback(
    async (quantity: number) => {
      if (quantity > liveAvailable) {
        showError(`Only ${liveAvailable} combo(s) left.`);
        return;
      }
      ...
      try {
        ...
        showSuccess(`${combo.name} has been added to your cart.`);
      } catch (err: any) {
    ...
    [combo._id, combo.name, dispatch, liveAvailable],
  );
    ...
    <ContainerComponent back title="About this menu">
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <SectionComponent styles={styles.content}>
              ...
              />
              <TextComponent
                text={combo.description || 'No description available.'}
                size={14}
                color="#666"
              ...
              />
              <FlatList
                data={filteredSuggestedCombos}
                keyExtractor={(item, index) => `Combo-${item._id || index}`}
                horizontal
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        navigateToItemDetail({
                          ...
                          itemType: 'Combo',
                          foodSizes: [],
                          allItems: filteredSuggestedCombos,
                        });
              ...
              <TextComponent
                text={`${avgRating.toFixed(
                  1,
                )} ⭐ Combo ratings (${totalRatings.toLocaleString()} reviews)`}
                size={16}
                font={appFonts.medium}
                styles={{ marginTop: 18 }}
              />
        ...
        ListEmptyComponent={
          isRatingLoading ? null : (
            <TextComponent
              text="No reviews available for this combo."
              size={14}
              color="#999"
      ...
      <View style={[styles.footer, isBusyLoading && { opacity: 0.97 }]}>
        ...
        <ButtonComponent
          ...
          styles={[
            styles.addBtn,
            (liveAvailable <= 0 || isDiscontinued) && {
              backgroundColor: '#ccc',
            },
            busy && { opacity: 0.7 },
          ]}
          ...
          onPress={
            busy || isDiscontinued || liveAvailable <= 0
              ? undefined
              : () => handleAddToCartPress(() => setQuantityModalVisible(true))
          }
          disable={busy || isDiscontinued || liveAvailable <= 0}
        />
      </View>
      ...
      {/* Loading modal: đang refresh / tải rating / tải gợi ý / thêm giỏ hàng */}
