...
import { updateAddressThunk } from '../../../redux/actions/addressAction';
...
<body>
  ...
  <div class="panel">
    <div class="card">
      ...
      <div class="title" id="title">Update Address</div>
      ...
      <div class="buttons">
        ...
        <button class="btn primary" id="saveBtn" onclick="saveAddress()" disabled>Save address</button>
      </div>
    </div>
  </div>
  ...
  <script>
    // ===== VN bounds helpers =====
    const B = { LAT_MIN: ${VN_BOUNDS.minLat}, LAT_MAX: ${
  VN_BOUNDS.maxLat
}, LNG_MIN: ${VN_BOUNDS.minLng}, LNG_MAX: ${VN_BOUNDS.maxLng} };
    const inVN = (lat,lng) => lat>=B.LAT_MIN && lat<=B.LAT_MAX && lng>=B.LNG_MIN && lng<=B.LNG_MAX;
    const nearestInVN = (lat,lng) => ({ lat: Math.min(Math.max(lat,B.LAT_MIN),B.LAT_MAX), lng: Math.min(Math.max(lng,B.LNG_MIN),B.LNG_MAX) });
  ...
  VN_BOUNDS.maxLng
    ...
    function setChips({lat,lng}) { chipLat.textContent='Lat: '+lat.toFixed(6); chipLng.textContent='Lng: '+lng.toFixed(6); }
    ...
    map.on('load', () => {
      ...
      marker = new maplibregl.Marker({ element: el, draggable: true }).setLngLat([${lng}, ${lat}]).addTo(map);

      marker.on('drag', () => { const ll = marker.getLngLat(); setChips({lat: ll.lat, lng: ll.lng}); });
      marker.on('dragend', () => {
        const ll = marker.getLngLat();
        window.ReactNativeWebView.postMessage(JSON.stringify({ type:'new_location', payload:{ latitude: ll.lat, longitude: ll.lng } }));
      });

      setChips({lat:${lat}, lng:${lng}});
    ...
    map.on('click', (e) => {
      ...
      window.ReactNativeWebView.postMessage(JSON.stringify({ type:'new_location', payload:{ latitude: e.lngLat.lat, longitude: e.lngLat.lng } }));
    });

    // ===== Geocoding VN (debounce) =====
    ...
    const searchByDetail = debounce(async () => {
      ...
      if (!r) return;
      let { lat, lng } = r;
      if (!inVN(lat, lng)) { const back = nearestInVN(lat,lng); lat=back.lat; lng=back.lng; }
      map.flyTo({ center:[lng,lat], zoom:16, essential:true, duration:800 });
      marker && marker.setLngLat([lng,lat]);
      setChips({lat,lng});
      window.ReactNativeWebView.postMessage(JSON.stringify({ type:'new_location', payload:{ latitude: lat, longitude: lng, source:'geocode' } }));
    }, 600);
    ...
    // Expose for RN: clamp về VN trước khi set
    window.updateMapLocation = function(lat, lng){
      let ll = { lat, lng };
      if (!inVN(lat,lng)) ll = nearestInVN(lat,lng);
      map.flyTo({ center:[ll.lng,ll.lat], zoom:16, essential:true, duration:800 });
      if (marker) { marker.setLngLat([ll.lng,ll.lat]); setChips(ll); }
    }

    // Nhận init để prefill label/detail/isDefault
    document.addEventListener('message', (event) => {
      try{
        const msg = JSON.parse(event.data||'{}');
        if (msg.type === 'init') {
          const p = msg.payload||{};
          if (typeof p.title === 'string') $title.textContent = p.title;
          if (typeof p.label === 'string') $label.value = p.label;
          if (typeof p.detail === 'string') $detail.value = p.detail;
          if (typeof p.isDefault === 'boolean') document.getElementById('defaultCheckbox').checked = p.isDefault;
          validate();
        }
      }catch(_){}
    });

    function requestLocationFromRN(){ window.ReactNativeWebView.postMessage(JSON.stringify({ type:'request_location' })); }
    ...
    function cancel(){ window.ReactNativeWebView.postMessage(JSON.stringify({ type:'cancel' })); }
  </script>
</body>
</html>
`;
...
export default function EditAddressScreen({ navigation }: any) {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(!addressToEdit);
  ...
  const onWebViewMessage = (event: any) => {
    try {
      ...
      switch (type) {
        case 'save': {
          const id = addressToEdit?._id;
          if (!id) {
            showModal('Missing address id', 'error', 'Error', [
              {
                label: 'Close',
                style: 'primary',
                onPress: () => setModalVisible(false),
          ...
          dispatch(updateAddressThunk({ id, data }))
            .then(unwrapResult)
            .then(() => {
              showModal(
  ...
  useEffect(() => {
    if (addressToEdit) {
      setSafeLocation(addressToEdit.latitude, addressToEdit.longitude);
      setIsLoading(false);
    } else {
      getCurrentPosition();
    }
  ...
  return (
    <ContainerComponent back title="Update Address">
      <View style={styles.container}>
        {isLoading || !currentLocation ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color={appColors.orange} />
            <TextComponent text="Loading map..." styles={styles.loadingText} />
          </View>
        ) : (
          <WebView
            key={webViewKey}
                ...
                webViewRef.current.postMessage(
                  JSON.stringify({
                    ...
                    payload: {
                      ...
                      detail: addressToEdit?.addressDetail ?? '',
